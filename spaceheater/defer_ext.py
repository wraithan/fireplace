...
class DictWrapper(object):
    ...
    def __getitem__(self, key):
        try:
            return self._wrap(self.data.__getitem__(key))
        except Exception:
            return FakeNone()

    def __repr__(self):
        if isinstance(self, (str, unicode, int, float)):
            raise Exception("Unexpected wrapped type")
...
class Defer(Extension):
    ...
    def _run(self, *args, **kwargs):
        ...
        output = DictWrapper(requests.get(url).json())
